...
from stockTwitsGraph.items import tweetData

class tweetSpider(scrapy.Spider):
	name = "tweetSpider"
	allowed_domains = ["stocktwits.com"]
	start_urls = ["http://stocktwits.com/RedDogT3"]
	
	def __init__(self):
		self.users = self.getUserList()
		self.start_urls = ["http://stocktwits.com/"+self.users[0]+"/"]
	...
	def getTweets(self,response):# get all tweets from first user page
		tweets = response.xpath(
			"//ol[@class='stream-list show-conversation stream-poller']/li/@data-src"
		).extract()
		...
		for tweet in tweets: # structure user and tweet data
			...
			data['sentiment'] 		   = rawData['sentiment']
