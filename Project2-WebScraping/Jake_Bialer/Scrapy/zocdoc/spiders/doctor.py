...
class DoctorSpider(CrawlSpider):
    ...
    start_urls = ['http://zocdoc.com/directory']
    rules = (
        Rule(LinkExtractor(allow=('directory/.+',)),  callback='parse_page'),
        Rule(LinkExtractor(allow=('(doctor|dentist)/.+',)),  callback='parse_doctor'),
        )


    def parse_page(self, response):
      base = "https://www.zocdoc.com/"
      all_links = response.xpath('*//a/@href').extract()
      doctors = filter(lambda x: "/doctor/" in x or "/dentist/" in x, all_links)
      directories = filter(lambda x: "/directory/" in x, all_links)
      for directory in directories: 
        yield scrapy.Request(base+directory, callback= self.parse_page)
      for doctor in doctors:
        yield scrapy.Request(base+doctor, callback= self.parse_doctor)
