shinyServer(function(input, output) {
  ...
  perc <- function(x) {
    a = na.omit(ehresp %>% group_by_(x) %>% summarise(b = n()) %>% mutate(c = round(100* b/sum(b))))
    return(paste(paste0(a[[3]],"%"), levels(ehresp[[x]]),collapse = " \n"))
  }
  assignplot <- function(v1, v2 = TRUE, v3 = TRUE){
    if (v2 == "None" & v3 =="None"){ 
      if (v1 %in% contvars){
        return(ngraphs)
      } else {
  ...
  dens2p <- function(v1, v2){
    return(ggplot(data = seldf(v1,v2),aes_string(x =v1)) + geom_density(aes_string(color = v2)))
  }
  ...
  output$h2 <- renderUI({
    selectizeInput(inputId = "h2", label = "Variable 2", selected = "wgt", choices = importantvars[!(importantvars %in% c(input$h1))])
  })
  output$h3 <- renderUI({
    if (input$h1 %in% catvars | input$h2 %in% catvars){
  ...
  output$f2 <- renderUI({
    selectizeInput(inputId = "f2", label = "Variable 2", choices = importantvars)
  })
  output$f3 <- renderUI({
    if (input$f1 %in% catvars | input$f2 %in% catvars){
  ...
  output$e2 <- renderUI({
    selectizeInput(inputId = "e2", label = "Variable 2", choices = importantvars)
  })
  output$e3 <- renderUI({
    if (input$e1 %in% catvars | input$e2 %in% catvars){
  ...
  output$m2 <- renderUI({
    selectizeInput(inputId = "m2", label = "Variable 2", choices = importantvars)
  })
  output$m3 <- renderUI({
    if (input$m1 %in% catvars | input$m2 %in% catvars){
  ...
  output$fo2 <- renderUI({
    selectizeInput(inputId = "fo2", label = "Variable 2", choices = importantvars)
  })
  output$fo3 <- renderUI({
    if (input$fo1 %in% catvars | input$fo2 %in% catvars){
  ...
  output$t2 <- renderUI({
    selectizeInput(inputId = "t2", label = "Variable 2", choices = importantvars)
  })
  output$t3 <- renderUI({
    if (input$t1 %in% catvars | input$t2 %in% catvars){
  ...
  output$i2 <- renderUI({
    selectizeInput(inputId = "i2", label = "Variable 2", selected = "wgt", choices = col_names[!(col_names %in% c(input$i1))])
  })
  output$i3 <- renderUI({
    if (input$i1 %in% catvars | input$i2 %in% catvars){
