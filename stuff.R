...
bigThree = VG2000 %>% group_by(companyPlatform) %>% summarise(Total_Sales = sum(Global_Sales), Total_Users = sum(User_Count, na.rm = T),
                                                              'NA' = sum(NA_Sales),
                                                              'EU' = sum(EU_Sales),
                                                              'JP' = sum(JP_Sales),
...
scaled_bigThree = scaled_bigThree[complete.cases(scaled_bigThree),]
...
VG2000 = VGdata[VGdata$Year_of_Release >= 2000,]
ggradar(scaled_bigThree) +  ggtitle('Regional Performance of Consoles')
View(scaled_bigThree)
View(bigThree)
View(tot_region_sales)

VGslice = VGdata[VGdata$Year_of_Release >= 2006 & VGdata$Year_of_Release<= 2008,]
View(VGslice)

ggplot(VG2000, aes(x=Critic_Score, y=NA_Sales)) +geom_point(
  aes(color = Genre)) + geom_smooth() + ylim(c(0,15)) + ggtitle("Review Scores versus Unit Sales(in millions)")


ggplot(data = subset(tot_region_sales, Year_of_Release >= 2000, Year_of_Release <= 2016),  aes(x = Year_of_Release)) + geom_line(aes(y=tot_NA_sales, colour = 'NA Sales'))+ geom_line(aes(y=tot_EU_sales, colour = 'EU Sales')) + geom_line(aes(y=tot_JP_sales, colour = 'JP Sales'))
