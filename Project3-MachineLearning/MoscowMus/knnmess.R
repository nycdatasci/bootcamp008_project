...
train_test$interest_level - 1

train_test[is.na(train_test)] <- 0

kcluster <- kmeans(train_test, centers = 3)
train_test <- cbind(train_test, kcluster$cluster)

train_test$bathrooms[train_test$bathrooms > 10] <- mean(train_test$bathrooms)
train_test$description_len[train_test$description_len > 1000] <- mean(train_test$description_len)

trainknn <- train_test[1:75000, ]
testknn <- train_test[75001:100000, ]
traintarget <- train_test[1:75000, 11]
testtarget <- train_test[75001:100000, 11]

m1 <- knn(train = trainknn, test = testknn, cl = traintarget, k = 50)
predtab <- table(testtarget, m1)
...
#perform training
gbdt1 = xgb.train(params = xgb_params,
                 data = dtrain,
                 nrounds =2425,
...
allpredictions1 =  (as.data.frame(matrix(predict(gbdt1,dtest), nrow=dim(test), byrow=TRUE)))
...
allpredictions2 =  (as.data.frame(matrix(predict(gbdt2,dtest), nrow=dim(test), byrow=TRUE)))
############################################NEW SEED ###################
...
#perform training
gbdt3 = xgb.train(params = xgb_params,
                  data = dtrain,
                  nrounds =5000,
                  watchlist = list(train = dtrain, val=dval),
                  print_every_n = 5,
...
allpredictions3 =  (as.data.frame(matrix(predict(gbdt3,dtest), nrow=dim(test), byrow=TRUE)))
...
train <- train_test[train_test$listing_id %in%train_id,]
test <- train_test[train_test$listing_id %in%test_id,]
test <- cbind(test, allpredictions3)
#Convert labels to integers
train$interest_level<-as.integer(factor(train$interest_level))
y <- train$interest_level
y = y - 1
...
#perform training
gbdt4 = xgb.train(params = xgb_params,
                  data = dtrain,
                  nrounds =5000,
                  watchlist = list(train = dtrain, val=dval),
                  print_every_n = 5,
...
allpredictions4 =  (as.data.frame(matrix(predict(gbdt4,dtest), nrow=dim(test), byrow=TRUE)))


######################
##Generate Submission
allpredictionscombined <- (.25 * allpredictions1) + (.25 * allpredictions2) + (.25 * allpredictions3) + (.25 * allpredictions4)
allpredictions = cbind(allpredictionscombined, test$listing_id)
...
imp <- xgb.importance(names(train),model = gbdt2)
xgb.ggplot.importance(imp)


names2 <- dimnames(train_test)[[2]]
importance_matrix1 <- xgb.importance(names2, model = gbdt4)
xgb.plot.importance(importance_matrix1[1:65,])
xgb.plot.tree(feature_names = names2, model = gbdt2, n_first_tree = 2)
